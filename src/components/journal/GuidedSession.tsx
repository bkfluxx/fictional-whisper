    ...
    return (
      <div className="flex flex-col h-full">
        <header className="flex items-center justify-between px-6 py-4 border-b border-foreground/10 shrink-0">
          ...
          <span className="text-sm font-medium text-foreground/60">Entry preview</span>
          <span className="w-24" />
        </header>

        <div className="flex-1 overflow-y-auto px-6 py-8 max-w-2xl mx-auto w-full">
          <p className="text-xs text-foreground/30 uppercase tracking-widest mb-4">
            Aura synthesised your session into a journal entry
          </p>

          <h2 className="text-2xl font-semibold text-foreground mb-3">{draft.title}</h2>
          ...
          {(draft.mood || (draft.categories && draft.categories.length > 0)) && (
            <div className="flex gap-2 flex-wrap mb-6">
              ...
              {draft.categories?.map((c) => (
                <span key={c} className="text-xs px-2.5 py-1 bg-foreground/10 text-foreground/50 rounded-full">
                  {c}
                </span>
              ))}
            </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => setPhase("chatting")}
              className="px-5 py-2.5 text-sm text-foreground/40 hover:text-foreground rounded-xl border border-foreground/10 hover:border-foreground/30 transition-colors"
            >
              Keep talking
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col h-full">
      <header className="flex items-center justify-between px-6 py-4 border-b border-foreground/10 shrink-0">
        ...
        <span className="text-sm font-medium text-foreground/60">Guided session</span>
        {canWrapUp ? (
          <button
            onClick={wrapUp}
          ...
          >
            ...
            >
              ...
              {m.role === "assistant" && streaming && i === messages.length - 1 && (
                <span className="inline-block w-1.5 h-4 bg-foreground/40 rounded-sm ml-0.5 animate-pulse align-middle" />
              )}
            </div>
          </div>
        ))}
      ...
      {/* Input area */}
      <div className="shrink-0 border-t border-foreground/10 px-4 py-4 max-w-2xl mx-auto w-full">
